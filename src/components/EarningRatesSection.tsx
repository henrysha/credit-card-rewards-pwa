  ...
  return (
    <div className="glass-card mt-md">
      ...
      {earningRates.map((rate, i) => {
        if (rate.quarterlySchedule) {
          const quarter = currentQuarterRewards(rate, now);
          return (
            <div key={i} data-testid="quarterly-categories">
              {quarter ? (
                <>
                  ...
                  <div className="quarterly-terms">
                    <div>{rate.limit} combined · Activation required</div>
                ...
                <div className="quarterly-terms">Current quarter categories not available. <a href="https://www.chase.com/personal/credit-cards/freedom/freedomfive" target="_blank" rel="noopener noreferrer">Check Chase ↗</a></div>
              )}
            </div>
          );
        }
